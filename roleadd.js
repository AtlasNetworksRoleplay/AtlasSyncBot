...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('roleadd')
        .setDescription('Adds role in all discords')
        .addUserOption(option =>
            option.setName('user')
                .setDescription('User to add the role too')
                .setRequired(true))
        .addRoleOption(option =>
            option.setName('role')
                .setDescription('The role to add in all servers')
                .setRequired(true)),
    async execute(config, interaction) {
        ...
        await interaction.reply(`Starting role assignment for **${targetUser.username}**...`);
        ...
        const summary = `✅ Role added in **${successCounter}** server(s).\n❌ Failed in **${failureCounter}** server(s).`;
